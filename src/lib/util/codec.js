...
const delimiter = "\0";
const typeReplacer = ["burst", "dps", "healer", "reaction", "shield", "sub-dps", "support"];
...
export function extractBuild(raw) {
  try {
    ...
    return {
      ...
      artifact: arr[5]
        .split(",")
        .map((a) => ({ set: a.split("|")[0].split("/"), p: a.split("|")[1] })),
      mainstat: {
        sand: arr[6]
          .split("|")[0]
          .split(",")
      ...
      source: arr[8].split("_!").map((s) => {
        ...
        return {
          [key]: source[1].replace(/_d/g, "default").replace(/_km/g, "https://keqingmains.com/"),
        };
      }),
