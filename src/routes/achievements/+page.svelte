  ...
  let filteredList = $derived.by(() => {
    ...
    tempList =
      currentFilter.field == ""
        ...
        : cloneDeep(
            tempList.filter(
              (achievement) => achievement[currentFilter.field] === currentFilter.value,
            ),
          );
    // 3. Sort
  ...
  function sortBy(list, sortField) {
    if (sortField === "achievements") {
      return list.sort((b, a) => {
        const aComplete = Object.values($localData["achievements"][a.achievement]).every(
          (c) => c === true,
        );
        const bComplete = Object.values($localData["achievements"][b.achievement]).every(
          (c) => c === true,
        );
    ...
    <div class="menu">
      {#each filters as filter}
        <a
          ...
          href="/#"
          onclick={preventDefault(() => (currentFilter = { field: "region", value: filter }))}
          >{ID[filter]}</a
        >
        {" · "}
      {/each}
      <a href="/#" onclick={preventDefault(() => (sortByCompletion = !sortByCompletion))}
        >Incomplete</a
      >
      <span class="menu-separator"></span>
